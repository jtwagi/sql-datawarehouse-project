   ...
   BEGIN TRY 
		...
		SELECT
			...
			CASE
				...
				ELSE 'N/A'
			END cst_material_status,-- Normalizing marital status to readable format 
			CASE
				WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
				WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
				ELSE 'N/A'
			END cst_gndr, -- Normalizing gender values to full names(readable format)
		cst_create_date
		FROM(
			SELECT *,
		...
		TRUNCATE TABLE silver.crm_prd_info;-- Emptying the table before insertion to avoid duplicate insertion
		...
		PRINT 'SILVER LOAD EXECUTION TIME: '+ CAST(DATEDIFF(second, @start_batch, @end_batch) AS NVARCHAR) + 'seconds';
	END TRY
	BEGIN CATCH
	   PRINT '=========================================';
